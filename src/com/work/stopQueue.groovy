...
node {
    ...
    stage('Stop SQS'){
        for (String serverName : map.keySet()) {
            def group = map.get(serverName)
            for (String v : group) {
                def stopSqs = sh(returnStdout: true, script: "ssh service@${v} curl -s -o /dev/null -I -w '%{http_code}' http://localhost:8080/sqs/stopQueue || true").trim()
                if(stopSqs != '200'){
                    error( v + " SQS STOP FAILED")
                }
            }
        }
    }

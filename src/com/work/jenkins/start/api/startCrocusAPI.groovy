...
node {
    ...
    def servers = null
    def tag = "Deploy"
    def value = "logistics-prd-crocus-api"
    def profile = "prod"
    ...
    stage ('Start') {
        ...
        servers.each{ server ->
            ...
            sh "ssh service@${server} java -jar -D7spring.application.name=crocus-api -Dspring.profiles.active=production -Dservice_mode=real -server -Xms2560m -Xmx2560m -XX:+UseG1GC -XX:MaxGCPauseMillis=200 -XX:G1HeapRegionSize=8m -XX:+ParallelRefProcEnabled -XX:-ResizePLAB -XX:+UseThreadPriorities -XX:ThreadPriorityPolicy=42 -Djasypt.encryptor.password=newlogistics /applications/build/${lastSuccessBuildID}/crocus-interfaces.jar 2>> /dev/null >> /dev/null &"
            ...
            for (i = 0; i <30; i++) {
                ...
                if(response == '200'){
                    ...
                    def sqsStart = sh(returnStdout: true, script: "ssh service@${server} curl -s -o /dev/null -I -w '%{http_code}' http://localhost:8080/queue/startQueue || true").trim()
                    ...
                    if(sqsStart != '200' || startKafka != '200'){
                        error(server + " START SQS/KAFKA Failed")
                    }

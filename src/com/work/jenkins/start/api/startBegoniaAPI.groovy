...
node {
    ...
    stage ('Start') {
        ...
        servers.each{ server ->
            ...
            sh "ssh service@${server} java -jar -Dspring.application.name=crocus-api -Dspring.profiles.active=production -Dservice_mode=real -server -Xms2560m -Xmx2560m -XX:+UseG1GC -XX:MaxGCPauseMillis=200 -XX:G1HeapRegionSize=8m -XX:+ParallelRefProcEnabled -XX:-ResizePLAB -XX:+UseThreadPriorities -XX:ThreadPriorityPolicy=42 -Djasypt.encryptor.password=newlogistics /applications/build/${lastSuccessBuildID}/crocus-interfaces.jar 2>> /dev/null >> /dev/null &"

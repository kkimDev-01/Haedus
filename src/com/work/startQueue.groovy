...
node {
    ...
    stage('Start Queue'){
        for (String serverName : map.keySet()) {
            def group = map.get(serverName)
            for (String v : group) {
                def statusSqs = sh(returnStdout: true, script: "ssh service@${v} curl -s -o /dev/null -I -w '%{http_code}' http://localhost:8080/sqs/startQueue || true").trim()

                if(statusSqs != '200'){
                    error( v + " SQS STATUS CHECK FAILED")
                    println('################ Status : '+statusSqs)
                } else {
                    println('################ Status : '+statusSqs)
                }
            }
